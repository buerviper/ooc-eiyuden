...
def post_status_with_image(name, spoiler_warning="False", status="#eiyuden #EiyudenChronicle #100HeroesStrong #jrpg #kickstarter", sensitivity="False", language="en"):
    description_file = open("images-anniversary/descriptions/" + name + ".yml", "r")
...
def media_description(media):
    description_file = open("images-anniversary/descriptions/" + media + ".yml", "r")
    description = yaml.load(description_file, Loader=yaml.FullLoader)
    alt_text = description["description"]
    os.chdir("images-anniversary")
...
photo = random.choice([x for x in os.listdir("images-anniversary") if os.path.isfile(os.path.join("images-anniversary", x))])

# Get name of photo without ending
name = Path("images-anniversary/" + photo).stem
...
if name[-1].isdigit():
    ...
    if os.path.exists("images-anniversary/" + name[:-1] + "3.png"):
        name_3 = name[:-1] + "3"
        name_list.append(name_3)
        if os.path.exists("images-anniversary/" + name[:-1] + "4.png"):
            name_4 = name[:-1] + "4"
