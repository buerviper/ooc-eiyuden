...
def post_status_with_image(name, spoiler_warning="False", status="#eiyuden #EiyudenChronicle #100HeroesStrong #jrpg #halloween #HappyHalloween", sensitivity="False", language="en"):
    description_file = open("images-halloween/descriptions/" + name + ".yml", "r")
...
def media_description(media):
    description_file = open("images-halloween/descriptions/" + media + ".yml", "r")
    description = yaml.load(description_file, Loader=yaml.FullLoader)
    alt_text = description["description"]
    os.chdir("images-halloween")
...
photo = random.choice([x for x in os.listdir("images-halloween") if os.path.isfile(os.path.join("images-halloween", x))])

# Get name of photo without ending
name = Path("images-halloween/" + photo).stem
...
if name[-1].isdigit():
    ...
    if os.path.exists("images-halloween/" + name[:-1] + "3.png"):
        name_3 = name[:-1] + "3"
        name_list.append(name_3)
        if os.path.exists("images-halloween/" + name[:-1] + "4.png"):
            name_4 = name[:-1] + "4"
